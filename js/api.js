function KundoAPI(slug) {
  // Instance variables
  this.BASE_URL = "https://kundo.se/api";
  this.FORMAT = ".json";
  if (!slug) {
  ...
  this.post_via_iframe = function(url, form, settings) {
    ...
    jQuery.receiveMessage(function(event){
      ...
      var status = "status" in data? data["status"]: null;
      delete data["status"];

      if (jQuery.isEmptyObject(data) || status == "CREATED") {
  ...
  this.GET = {
    ...
    single: function(dialog_id, settings) {
      if (!dialog_id || !that.isInt(dialog_id)) {
  ...
  this.POST = {
    ...
    comment: function(form, dialog_id, settings) {
      ...
      else if (!dialog_id || !that.isInt(dialog_id)) {
        throw new Error("Invalid dialog id. It should be numeric.");
      }
      else if (!settings || !settings.success || !settings.error) {
        throw new Error("You need to specify both a success and error callback");
      }
    ...
    report_comment: function(form, comment_id, settings) {
      ...
      else if (!comment_id || !that.isInt(comment_id)) {
